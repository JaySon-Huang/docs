...
TiFlash has two components: the columnar storage module and `tiflash proxy`. `tiflash proxy` is responsible for the communication using the Multi-Raft consensus algorithm.

When TiDB receives the DDL command to create replicas in TiFlash, TiDB acquires the information of the table to be replicated, and add the placement-rules to PD. Then PD performs the corresponding data scheduling.
